...
class PobierzUliceKINAImpl(object):
    WFS_URL = "https://mapy.geoportal.gov.pl/wss/ext/wfs/KrajowaIntegracjaNumeracjiAdresowej"
    DEFAULT_TILE_SIZE = 2500

    # Pola w 100% zgodne z atrybutami warstwy A08_Ulice oraz nazwami powiatu i województwa
    FIELDS_DEFINITION = [
    ...
    def getParameterInfo(self):
        ...
        param_out_name.value = "Pobrane_Ulice_KINA"
    ...
    def _extract_attr_value(self, attrs, f_name, context_woj=None, context_pow=None, context_gmi=None, lookups=None):
        ...
        mapping = {
            ...
            "NAZWA_MIEJSCOWOSCI": ["nazwa_miejscowosci", "miejscowosc", "czmiejscowosci"],
            "ID_MIEJSCOWOSCI": ["id_miejscowosci", "id_miejsc", "sym_miejsc", "sym"],
            "NAZWA_ULICY": ["nazwa_ulicy", "nazwaglowyczon", "nazwaglowa", "ulica", "nazwa"],
    ...
    def _parse_gml_lines(self, xml_bytes, errors):
        try:
            root = ET.fromstring(xml_bytes)
        except Exception as e:
            errors.append(f"Parsowanie GML linii ulic: {e}")
            return []
        ...
        for el in root.iter():
            ...
            local_l = local.lower()
            is_direct_feature = "ulic" in local_l
            if local in ("member", "featureMember") or is_direct_feature:
                ...
                for node in feat_node.iter():
                    ...
                    if tag in ("poslist", "coordinates") and node.text:
                        coords = [float(c) for c in re.findall(r"[-+]?\d*\.\d+|\d+", node.text)]
                        if len(coords) >= 4:
                            pts = []
                            for idx in range(0, len(coords), 2):
                                x, y = coords[idx+1], coords[idx]  # Northing, Easting -> X, Y
                                pts.append(f"{x} {y}")
                            wkt_geom = "LINESTRING (" + ", ".join(pts) + ")"
    ...
    def execute(self, p, messages):
        ...
        if not arcpy.Exists(out_fc):
            arcpy.CreateFeatureclass_management(out_ws, out_name, "POLYLINE", spatial_reference=sr_2180)
        ...
        candidate_typenames = [self.FALLBACK_TYPENAME]
        try:
            ...
            for n in found_names:
                if "ulic" in n.lower():
                    clean_n = n.strip()
                    if clean_n not in candidate_typenames:
                        candidate_typenames.insert(0, clean_n)
        except Exception:
            pass

        actual_typename = self.FALLBACK_TYPENAME
        if tiles:
            probe_x1, probe_y1, probe_x2, probe_y2 = tiles[0]
            probe_bbox = f"{probe_y1},{probe_x1},{probe_y2},{probe_x2},urn:ogc:def:crs:EPSG::2180"
            for candidate in candidate_typenames:
                probe_params = {
                    "SERVICE": "WFS",
                    "REQUEST": "GetFeature",
                    "VERSION": "1.1.0",
                ...
                probe_url = f"{self.WFS_URL}?{urllib.parse.urlencode(probe_params)}"
                try:
                    probe_resp = HttpClient.fetch_text(probe_url, timeout=25)
                    if "ExceptionReport" not in probe_resp and "ServiceException" not in probe_resp:
                        actual_typename = candidate
                        break
                except Exception:
                    continue
        ...
        with arcpy.da.InsertCursor(out_fc, insert_fields) as cursor:
            for idx, (x1, y1, x2, y2) in enumerate(tiles, start=1):
                ...
                try:
                    ...
                    items = self._parse_gml_lines(xml_data, errors)
                    ...
                    for wkt_str, attrs in items:
                        ...
                        try:
                            ...
                            errors.append(f"Zapis ulicy {unique_id}: {ins_err}")
        ...
        arcpy.AddMessage(f"Faza 3/3: Dodano nowych ulic: {len(processed_ids) - initial_count}")
